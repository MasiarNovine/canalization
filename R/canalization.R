...
get_indices <- function(age, ref, sex=c('male', 'female')) {
    ...
    for (i in 1:nrow(ref)) {
        ...
        if (delta < 0) {
            # This takes care of the case, if the reference does not contain values for age 0
            # We jump in the next available time period, which is not quite as accurate
...
#' }
#' \details{
#' A check function to look for mandatory variables names in the data object used as an input.
#' }
#' \arguments{
...
#' \name{build_age_cohorts}
#' \alias{build_age_cohorts}
...
#'   subjects are non-obese concerning their BMI-SDS. The result is a list with two `data.frames`: 
#'   One with the extracted values for the at-risk (non-obese) and one for the at-risk (obese) 
#'   group. The idea is to have a projective outlook on the samples based on the respective   
...
check_mixed_model <- function(model, cresc_data, seed = 1) {
    ...
    rndidx <- sample(cresc_data[, unique(subject_id)], size = min_size)
    
    # ASSUMPTION 1: CONSTANT VARIANCE 
    # Check, whether the expection is 0 and the variability is constant
    p1 <- ggplot(cresc_data[subject_id %in% rndidx, ], aes(subject_id, resid)) + 
        geom_boxplot() +
        coord_flip() +
    ...
    p2 <- ggplot(cresc_data[subject_id %in% rndidx, ], aes(x = fitted, y = resid, group = subject_id)) + 
    ...
    p3 <- ggplot(cresc_data[subject_id %in% rndidx, ], aes(x = age, log(weight))) + 
        geom_point() +
...
make_predictions <- function(model, cresc_data, seed = 1, log = TRUE) {
    ...
    g <- if (log) {
        ...
        ggplot(cresc_test, aes(x = age, weight))
    }
